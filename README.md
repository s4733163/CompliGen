...
1. **Web Server**: Deploy with Gunicorn + Nginx
   ```bash
   gunicorn CompliGen.wsgi:application --bind 0.0.0.0:8000
   ```

2. **Database**: Use managed PostgreSQL (AWS RDS, DigitalOcean, Supabase)

3. **Environment Variables**: Use secrets manager (AWS Secrets Manager, Vault)

4. **HTTPS**: Enable SSL/TLS with Let's Encrypt

5. **Static Files**: Configure `STATIC_ROOT` and use `collectstatic`

6. **Settings**: Create `settings_prod.py` with:
   - `DEBUG = False`
...
2. **Deploy**: Host on Vercel, Netlify, or CloudFlare Pages

3. **Environment Variables**: Set API base URL for production backend

**ChromaDB**:
1. **Persistent Storage**: Mount volume or use cloud storage
2. **Alternative**: Consider hosted vector database (Pinecone, Weaviate)
...
services:
  ...
  backend:
    build: ./CompliGen
    command: gunicorn CompliGen.wsgi:application --bind 0.0.0.0:8000
    volumes:
      - ./CompliGen:/app
    ...
    depends_on:
      - db
    environment:
      - DATABASE_URL=postgresql://postgres:<password>@db:5432/compligen_db

  frontend:
    build: ./CompliGen_Frontend
    ports:
      - "80:80"
    depends_on:
      - backend
